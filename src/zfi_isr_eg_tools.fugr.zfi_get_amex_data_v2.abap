...
*"----------------------------------------------------------------------
  data: it_paso0       TYPE ZFI_TRASPASOST, "Docs que involucren al proveedor amex
        ...
        it_paso0C      TYPE ZFI_TRASPASOST, "Docs de compensación a bancos
  ...
  perform get_general_data_V2 using it_paso0
                                    it_paso0B
                                    it_paso0C
                                    it_paso1
                                    it_paso2
...
* Busca el rfc de proveedor
  ...
  if it_paso0C is NOT INITIAL.
    ...
    SELECT ktopl, saknr,txt50
                 ...
                 WHERE spras = @sy-langu AND
                       ktopl = @it_paso0c-ktopl AND
                       saknr = @it_paso0c-racct.

* Busca el rfc de proveedor
    SELECT partner,taxtype,taxnum,taxnumxl
      APPENDING TABLE @tl_dfkkbptaxnum
      FROM dfkkbptaxnum
      FOR ALL ENTRIES IN @it_paso0c
      WHERE partner  = @it_paso0c-lifnr AND
            taxtype  = @c_mx1.

    SELECT partner,name_org1,name_org2,name_org3,name_org4,name_last,name_first,natpers
      APPENDING TABLE @tl_but000
      FROM but000
      FOR ALL ENTRIES IN @it_paso0c
      WHERE partner  = @it_paso0c-lifnr.
...
***************************************DATOS DE DOCUMENTO DE PAGO***************************************
  loop at it_paso0b into DATA(iw_paso0B). "Por Compensación.

    read TABLE it_paso0c into DATA(iw_paso0c) with key  RLDNR  = iw_paso0b-rldnr
                                                          RBUKRS = iw_paso0B-RBUKRS
                                                          GJAHR  = iw_paso0B-GJAHR
                                                          BELNR  = iw_paso0B-BELNR.

    if sy-subrc = 0.
*        Obtnere nombre del cliente
      ASSIGN tl_but000[ partner = iw_paso0c-lifnr ] TO FIELD-SYMBOL(<fs_but000>).
      IF sy-subrc = 0.
      ...
      ENDIF.
*ACREDOR
      REPORT_OUTPUTL-ACREDOR = iw_paso0c-lifnr.
      READ TABLE  tl_dfkkbptaxnum into DATA(wl_dfkkbptaxnum) WITH key partner  = iw_paso0c-lifnr
                                                                  taxtype      = c_mx1.
      if sy-subrc = 0.
    ...
    REPORT_OUTPUTL-TOTALCARGOS = iw_paso0B-HSL.
    ...
    perform datos_doc_pago USING REPORT_OUTPUTL
                             iw_paso0B.
    REPORT_OUTPUTL-N_DOC_PAGO = iw_paso0B-belnr.
    REPORT_OUTPUTL-OTROS_RETIROS =  REPORT_OUTPUTL-OTROS_RETIROS + iw_PASO0b-hsl.
    REPORT_OUTPUTL-OTROS_RETIROS = REPORT_OUTPUTL-OTROS_RETIROS * ( -1 ).
    REPORT_OUTPUTL-NEW_REPORT = 'X'.
    REPORT_OUTPUTL-DESCRIPCION = iw_paso0B-SGTXT.
    REPORT_OUTPUTL-CLASIFICACION = text-050.
    if iw_paso0B-XBLNR is not initial.
      REPORT_OUTPUTL-REFERENCIA = iw_paso0B-XBLNR.
    else.
      REPORT_OUTPUTL-REFERENCIA = iw_paso0B-AWKEY.
    endif.
    append REPORT_OUTPUTL to REPORT_OUTPUT.

  endloop.
***************************************DATOS DE DOCUMENTO DE PAGO***************************************
  DESCRIBE TABLE it_paso2 LINES DATA(size_p).
  ...
  loop at  it_paso1 into DATA(iw_paso1).
    ...
    loop at it_paso2 into DATA(iw_paso2) where  RLDNR  = iw_paso1-rldnr and
                                                RBUKRS = iw_paso1-RBUKRS and
                                                AUGGJ  = iw_paso1-AUGGJ and
                                                AUGBL  = iw_paso1-AUGBL.
      index = sy-tabix.
      read TABLE it_paso0 into DATA(iw_paso0) WITH key  RLDNR  = iw_paso1-rldnr
                                                        RBUKRS = iw_paso1-RBUKRS
                                                        GJAHR  = iw_paso1-gjahr
                                                        BELNR  = iw_paso1-belnr.
      if index > 1 and iw_paso0-augbl is initial.
...
*RAZON_S
*        Obtnere nombre del cliente
            ASSIGN tl_but000[ partner = iw_paso5-lifnr ] TO <fs_but000>.
            IF sy-subrc = 0.
              IF <fs_but000>-natpers IS INITIAL.
                CONCATENATE <fs_but000>-name_org1 <fs_but000>-name_org2 <fs_but000>-name_org3 <fs_but000>-name_org4 INTO REPORT_OUTPUTL-razon_s SEPARATED BY space.
              ELSE.
...
*ACREDOR
            ...
            READ TABLE  tl_dfkkbptaxnum into wl_dfkkbptaxnum WITH key partner  = iw_paso5-lifnr
                                                                      taxtype      = c_mx1.
            if sy-subrc = 0.
              if wl_dfkkbptaxnum-TAXNUM is NOT INITIAL.
                REPORT_OUTPUTL-RFC = wl_dfkkbptaxnum-TAXNUM.
...
*N_DOCUMENTO_FAC_SOCIEDAD
        REPORT_OUTPUTL-N_DOCUMENTO_FAC_SOCIEDAD = iw_paso4-RBUKRS.
*N_DOCUMENTO_FAC_SOCIEDAD
        REPORT_OUTPUTL-N_DOCUMENTO_FAC_EJERCICIO = iw_paso4-GJAHR.
*N_DOCUMENTO_FAC
...
*       BASE_NO_OBJ
        IF iw_paso4-mwskz IS INITIAL AND iw_paso4-ktosl NE 'WIT'.
          IF iw_paso4-hsl LT 0.
            iw_paso4-hsl = iw_paso4-hsl * -1.
          ENDIF.
          REPORT_OUTPUTL-base_no_obj    = iw_paso4-hsl.
        ENDIF.
*ACTIVO_FIJO1
